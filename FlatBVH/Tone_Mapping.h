...
static vec3 Filmmic_ToneMapping(const vec3& c)
{
...
vec3 Filmic_yblein(const vec3& v)
{
	vec3 x = max_vector(v - vec3(0.004), vec3(0.0f));
...
vec3 ACES_Tone_Mapping(const vec3& color)
{
	vec3 c(color);
