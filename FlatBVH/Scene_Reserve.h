...
struct Scene
{
	...
	Scene(const string& filename, const string& enviroment_path = "", const float& rotation = 0)
	{
		//ReadMtl(filename);
		ReadObj(filename);
		//LightSize = Ls.Ls.size();
		//if (LightSize)
		//	pdfLight = 1.0f / LightSize;
		//else
		//	pdfLight = 0.5f;

		/*if (enviroment_path != "")
		{
		enviroment = EnvLight(enviroment_path, rotation);
		use_eviroment = false;
		}
		else
		use_eviroment = true;
	...
	//unordered_map<int, int> texMap;
	//unordered_map<int, int> triangle_light;
	//unordered_map<int, int> light_triangle;
		...
		while (si >= 0)
		{
				...
				//if(top->box.hit_axis_tl_mint(r, tl, mint))
			{
				...
				int sort_axis = r.sign[top.axis];//1
												 ...
												 //32.49s
				stack[si + 1] = top.start;//top.nodes[sort_axis];//1
				stack[si + 2] = top.start + 1;//top.nodes[1 - sort_axis];//0//32.49
		...
		while (si >= 0)
		{
			...
			if (top->box.hit_axis_tl(r, tl) && tl < mint)
			{
				if (!top->leaf)
				{
					...
					stack[si + 1] = top->nodes[sort_axis];//1
					stack[si + 2] = top->nodes[1 - sort_axis];//0
					si += 2;
				}
				else
				{
	...
	bool __fastcall hit_color(node*& n, Ray& r, HitRecord& rec, vec3* color)
	{
		node* stack[32];
		...
		while (si >= 0)
		{
				...
				//if(top->box.hit_axis_tl_mint(r, tl, mint))
			{
				...
				int sort_axis = r.sign[top->axis];//1
												  ...
												  //32.49s
				stack[si + 1] = top->nodes[sort_axis];//1
				stack[si + 2] = top->nodes[1 - sort_axis];//0//32.49
	...
	bool __fastcall hit_anything_range(node*& n, Ray& r, const float& d)
	{
		...
		while (si >= 0)
		{
				...
				//if(top->box.hit_shadow(r))
			{
				...
				{
					//34s
					stack[si + 1] = top->nodes[0];
					stack[si + 2] = top->nodes[1];
					...
					//float p = randf();
					//int node = p < 0.5f;
					//stack[si + 1] = top->nodes[node];
					//stack[si + 2] = top->nodes[1 - node];

					//35.37s
					//int s = r.sign[top->axis];
					//stack[si + 1] = top->nodes[s];
					//stack[si + 2] = top->nodes[1 - s];

					//33.65s
					//int s = r.sign[top->axis];
					//stack[si + 1] = top->nodes[1 - s];
					//stack[si + 2] = top->nodes[s];

					//first axis
					//bath room 31.629s
					//49.323s
					//stack[si + 1] = top->nodes[first_axis];
					//stack[si + 2] = top->nodes[1 - first_axis];


					//48.558s sibenik 33.5s bathroom
					//stack[si + 1] = top->nodes[1 - first_axis];
					//stack[si + 2] = top->nodes[first_axis];

					//49.244 sibenik 31.613s bathroom
					//stack[si + 1] = top->nodes[first_axis];
					//stack[si + 2] = top->nodes[1 - first_axis];
	...
	float __fastcall mis3(const float& pdf1, const float& pdf2)
	{
		float pdf1_3 = pdf1 * pdf1 * pdf1;
		return pdf1_3 / (pdf1_3 + pdf2 * pdf2 * pdf2);
	}

	float __fastcall mis2_2(const float& pdf1, const float& pdf2)
	{
		float pdf1_2_2 = powf(pdf1, 2.2f);
		return pdf1_2_2 / (pdf1_2_2 + powf(pdf2, 2.2f));
	}
	...
	vec3 __fastcall path_tracing(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 T(1.0f);
		HitRecord rec;
