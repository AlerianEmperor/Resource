...
const float sigma_s_ = 0.06;//0.06;//0.04;//0.09;//0.036;//0.036//0.006;//0.036;//0.009;//0.05
const float sigma_s_sun = 0.036;//0.05;//0.036;
const float sigma_a_sun = 0.02;//0.6;//0.4;//0.2 test 2 //0.002 original test
const float sigma_t_sun = sigma_s_sun + sigma_a_sun;
const float sigma_h_ = 1.0f;
const float g_ = -0.5;
//vec3 sun_color_god_ray = vec3(9, 6, 3);//3 2 1
...
struct Sphere
{
	...
	double rad_square;
	vec3 p, c;// , e;
	unsigned int mtl_type = Glass_Fresnel_type;
	Sphere(double rad_, vec3 p_, vec3 c_, vec3 e_, int mtl_type_) : r(rad_), rad_square(rad_ * rad_), p(p_), c(c_), mtl_type(mtl_type_) {}
...
//Sphere sph(0.6, vec3(-1.78, 4.74177, -2.79818), vec3(0.0f, 1.0f, 0.0f), vec3(0.0f), Glass_Fresnel_type);

//Sphere sph(0.6, vec3(-3.78, 1.74177, -0.79818), vec3(0.0f, 1.0f, 0.0f), vec3(0.0f), Glass_Fresnel_type);

Sphere sph(0.6, vec3(-3.78, 1.74177, -1.79818), vec3(0.0f, 1.0f, 0.0f), vec3(0.0f), Glass_Fresnel_type);
...
struct Scene
{
	...
	Scene(const string& filename, bool use_area_light_, bool use_directional_light_, vec3 light_direction_, float directional_blur_radius_ = 0.0f, bool use_sun_color_ = true, bool compute_sky_color_ = true, string enviroment_path = "", float rotation = 0)
	{
		...
		infinite_light_direction = light_direction_.norm();

		original_sun_direction = light_direction_.norm();
	...
	float directional_blur_radius = 0.02f;

	//Volumetric Room
	//vec3 sun_color = vec3(3, 2, 1);

	//Class Room
	//vec3 sun_color = vec3(90, 60, 30);
	...
	vec3 infinite_light_direction = (-1, -0.1, 0.2);
	vec3 original_sun_direction = vec3(1.0f, 0.4f, 0.2f);
	...
	void ReadMtl(const string& filename, unordered_map<string, int>& mtlMap, const string& EnviromentName = "")//mtlMap chi dung cho giai doan doc file, ender thi khong can)
	{
		...
		while (f.getline(line, 256))
		{
			...
			else if (strncmp(t, "type", 4) == 0)
			{
				...
				else if (strncmp(type, "GlassRealistic", 14) == 0)
					mats[countMaterial].type = Glass_Realistic_type;
				else if (strncmp(type, "GlassFresnel", 12) == 0)
					mats[countMaterial].type = Glass_Fresnel_type;
				else if (strncmp(type, "GlassBeerLaw", 12) == 0)
					mats[countMaterial].type = Glass_Beer_Law_type;
				else if (strncmp(type, "Glass ", 6) == 0)
				...
				else if (strncmp(type, "SubSurface", 10) == 0)
				{
					//cout << "Leu Leu\n";
					mats[countMaterial].type = SubSurface_Scattering_type;
				}
			}
		...
		for (int i = 0; i < numberOfMaterial; ++i)
		{
			if (!mats[i].isLight)
			{
				if (mats[i].type == Diffuse_type)
					bsdfs[i] = new Diffuse();
				if(mats[i].type == Orenayar_type)
					bsdfs[i] = new Oren_nayar();
				...
				else if (mats[i].type == Glass_Realistic_type)
					bsdfs[i] = new Glass_Realistic(mats[i].ior);
				else if (mats[i].type == Glass_Fresnel_type)
					bsdfs[i] = new Glass_Fresnel(mats[i].ior);
				else if (mats[i].type == Glass_Beer_Law_type)
				{
					//cout << "Good Beer";
					bsdfs[i] = new Glass_Beer_Law(mats[i].ior, mats[i].density, mats[i].Kd);
				...
				else if(mats[i].type == SubSurface_Scattering_type)
					bsdfs[i] = new Diffuse();
			}
		}
	}
	...
	bool __fastcall hit_color_alpha(node*& n, Ray& r, HitRecord& rec, vec3* color, float& alpha)
	{
		...
		if (mint < 10000000.0f && mint >= eps)//1e9f)
		{
			rec.t = mint;
			...
			if (top->box.hit_shadow_th(r, th))// && th >= 0.0f)
			{
				...
				else
				{
					...
					for (auto i = start; i < end; ++i)
					{
						...
						HitRecord rec;
						if (trs[i].hit_anything_alpha(r, rec) && 1.000024f * rec.t <= d)
						{
							if (!mats[mtl].alpha_material)
								return true;
							else
	...
	vec3 __fastcall sibenik_tracing_directional_lighting_2(node*& n, Ray& r)
	{
		...
		for (int i = 0; i < 60; ++i)
		{
			...
			if (hit_color_alpha(n, new_ray, rec, &color, Tr))
			{
				...
				{
					//cout << "a";
					vec3 sun_direction = infinite_light_direction;
	...
	vec3 __fastcall sibenik_tracing_directional_lighting_2_God_Ray(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 T(1.0f);
		HitRecord rec;
			...
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
				...
				if (!bsdfs[mtl]->isSpecular())
				{
					if (use_directional_light)
					{
						...
						//Ray directional_lighting(hit_point, infinite_light_direction);
						Ray directional_lighting(hit_point, sun_direction);

						if (!hit_anything_range(n, directional_lighting))
						//float dist = 1000;
						//if(!hit_anything_range_alpha(n, directional_lighting, dist))
						{
							//cout << "yes\n";
							vec3 directional_bsdf = bsdfs[mtl]->eval(new_ray.d, sun_direction, rec.n, original_normal, color);

							//MIS
							//float directional_pdf = bsdfs[mtl]->pdf(new_ray.d, infinite_light_direction, rec.n, original_normal);
							//float sun_pdf = 1.0f;//there is only one way to sample this direction
	...
	vec3 __fastcall sibenik_tracing_directional_lighting_NEE_scatter(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 T(1.0f);
		HitRecord rec;
				...
				if (!bsdfs[mtl]->isSpecular())
				{
					if (use_directional_light)
					{
							...
							//if(!hit_anything_range_alpha(n, directional_lighting, dist))
						{
							...
							if (isScatter(light_distance))
							{
								vec3 hit_point(new_ray.o + new_ray.d * light_distance);

								vec3 Transmitance(expf(-sigma_t * light_distance));
					...
					if (use_area_light)
					{
						...
						/*if (isScatter(length))
						{
							vec3 hit_point(new_ray.o + new_ray.d * length);

							vec3 Transmitance(expf(-sigma_t * length));
	...
	vec3 __fastcall sibenik_tracing_directional_lighting_NEE_scatter_2(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 T(1.0f);
		HitRecord rec;
		...
		for (int i = 0; i < 60; ++i)
		{
			...
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
				...
				float distance = (new_ray.d * rec.t).length();

				//float cont_prob = 1.0f;

				if (isScatter2(distance))
				{
					vec3 hit_point(new_ray.o + new_ray.d * distance);

					vec3 Transmitance(expf(-sigma_a * distance));
				...
				if (!bsdfs[mtl]->isSpecular())
				{
					if (use_directional_light)
					{
							...
							//if(!hit_anything_range_alpha(n, directional_lighting, dist))
						{
							...
							if (isScatter2(light_distance))
							{
								//cout << "scatter";
								vec3 hit_point(new_ray.o + new_ray.d * light_distance);

								vec3 Transmitance(expf(-sigma_a * light_distance));
								...
								vec3 direction_out;
								float pdf_scatter = HG_Sample(-new_ray.d, direction_out);

								float pdf_light = 1.0f / (pi * directional_blur_radius * directional_blur_radius);//0.1f;
								
								
								L += T * (pdf_scatter * pdf_scatter) / (pdf_scatter * pdf_scatter + pdf_light * pdf_light);
	...
	vec3 __fastcall sibenik_tracing_directional_lighting_NEE_scatter_original(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 T(1.0f);
		HitRecord rec;
		...
		for (int i = 0; i < 60; ++i)
		{
			...
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
				...
				float atmos_pdf = (Transmitance.x + Transmitance.y + Transmitance.z) / 3;
					...
					if (use_directional_light)
					{
							...
							//if(!hit_anything_range_alpha(n, directional_lighting, dist))
						{
							...
							if (isScatter2(light_distance))
							{
								...
								float atmos_pdf = (density.x + density.y + density.z) / 3; //density.x;
								...
								prev_pdf *= pdf_scatter;

								L += sun_color * T * (pdf_scatter * pdf_scatter) / (pdf_scatter * pdf_scatter + pdf_light * pdf_light);
	...
	vec3 __fastcall sibenik_tracing_directional_lighting_NEE_scatter_different_sigma_sun(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 scatter_T(1.0f);
		vec3 T(1.0f);
				...
				if (!bsdfs[mtl]->isSpecular())
				{
					if (use_directional_light)
					{
							...
							//if(!hit_anything_range_alpha(n, directional_lighting, dist))
						{
							...
							float light_distance = 4.0f;

							//if (isScatter2(light_distance))
							if(isScatter2(distance, sigma_s_sun))
							{
								//cout << "scatter";
								vec3 hit_point(new_ray.o + new_ray.d * light_distance);

								vec3 Transmitance(expf(-sigma_a_sun * light_distance));
								...
								L = T * sun_color * (pdf_light * pdf_light) / (pdf_scatter * pdf_scatter + pdf_light * pdf_light) / pdf_light;
	...
	vec3 __fastcall god_ray(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 T(1.0f);
		...
		float scatter_prob = 0.06f;

		float inv_scatter_prob = 1.0f / scatter_prob;
		float inv_1_scatter_prob = 1.0f / (1.0f - scatter_prob);

		float sigma_sun_s = 0.06f;
		float sigma_sun_a = 0.02f;
		...
		for (int i = 0; i < 60; ++i)
		{
			...
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
				...
				if (isScatter2(distance, sigma_s))
				{
					vec3 hit_point(new_ray.o + new_ray.d * distance);
				...
				else
				{
					...
					//what is the value of sigma_s so that it will give 46% scatter probability

					//-log(randf()) / sigma_s_sun = 0.45
					//sigma_s_sun * 0.45 = (sum-log(randf()))

					if (randf() < scatter_prob)
					{
						float scatter_distance = distance * randf();
						...
						T *= expf(-scatter_distance * sigma_a);

						T *= inv_scatter_prob;


						if (!hit_anything_range(n, sun_ray))
						{

							float pdf_scatter = HG_pdf_sun(new_ray.d, sun_direction, g_sun);
							float pdf_light = 1.0f / (pi * directional_blur_radius * directional_blur_radius);

							float weight = pdf_light * pdf_light / (pdf_scatter * pdf_scatter + pdf_light * pdf_light);

							L += T * sun_color * weight / pdf_light;

							vec3 scatter_direction;
							float pdf = HG_Sample_sun(new_ray.d, scatter_direction, g_sun);

							prev_pdf = pdf;

							new_ray = Ray(hit_point, scatter_direction);
						...
						{
							vec3 scatter_direction;
							float pdf = HG_Sample(new_ray.d, scatter_direction);
					...
					{
						T *= expf(-sigma_a * distance);
						T *= inv_1_scatter_prob;
						...
						if (!hit_anything_range(n, sun_ray))
						{
							...
							float directional_pdf = bsdfs[mtl]->pdf(new_ray.d, infinite_light_direction, rec.n, original_normal);
							float sun_pdf = 1.0f / (pi * directional_blur_radius * directional_blur_radius);
	...
	vec3 __fastcall god_ray_2(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 scatter_T(1.0f);
		vec3 T(1.0f);
		...
		float scatter_prob = 0.26f;
		float sigma_sun_s = 0.6f;
		float sigma_sun_a = 0.002f;
		float g_sun = -0.5f;

		float inv_scatter = 1.0f / scatter_prob;
		float inv_1_scatter = 1.0f / (1.0f - scatter_prob);

		float sun_scale = 0.001f;
			...
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
				...
				if (!bsdfs[mtl]->isSpecular())
				{
					if (use_directional_light)
					{
						...
						if (randf() < scatter_prob)
						{
							float scatter_distance = randf() * distance;

							T *= expf(-sigma_a_sun * scatter_distance);
							//T *= inv_scatter;

							vec3 scatter_point = new_ray.o + new_ray.d * scatter_distance;

							Ray directional_lighting(scatter_point, sun_direction);

							if (!hit_anything_range(n, directional_lighting))
							{
								float scatter_pdf = HG_pdf_sun(new_ray.d, sun_direction, g_sun);
								float light_pdf = 1.0f / (pi * directional_blur_radius * directional_blur_radius);

								float weight = light_pdf * light_pdf / (scatter_pdf * scatter_pdf + light_pdf * light_pdf);

								L += T * sun_color * weight / light_pdf * sun_scale;
							}

							vec3 direction_out;
							
							float pdf = HG_Sample_sun(new_ray.o, direction_out, g_sun);

							//prev_pdf = pdf;

							new_ray = Ray(scatter_point, direction_out);
							...
							//if(!hit_anything_range_alpha(n, directional_lighting, dist))
						{
							...
							if (isScatter2(distance, sigma_s_sun))
							{
								...
								L = T * sun_scale * sun_color * (pdf_light * pdf_light) / (pdf_scatter * pdf_scatter + pdf_light * pdf_light) / pdf_light;
	...
	vec3 estimate_direct_lighting(node*& n, vec3& direction_in, vec3& hit_point, vec3& normal, vec3& original_normal, int mtl, int type)
	{
		vec3 sun_direction = infinite_light_direction;

		vec3 L(0.0f);
		directional_blur_radius = 0.2f;
		if (blur_directional_light)
		{
			vec3 blur_center = hit_point + sun_direction;
		...
		if (type == Volume_type)
		{
			...
			Ray light_ray(hit_point, sun_direction);

			//float d = 1000;
		
			//if (!hit_anything_range_alpha(n, light_ray, d))
			if(!hit_anything_range(n, light_ray))
			{
				//cout << "hit";
				float scatter_pdf = HG_pdf(direction_in, sun_direction);

				vec3 f(scatter_pdf);

				float weight = pdf_light * pdf_light / (pdf_light * pdf_light + scatter_pdf * scatter_pdf);

				L += f * weight / pdf_light;
			}
		}
		else
		{
			...
			{
				//cout << "hit";
				float scatter_pdf = bsdfs[mtl]->pdf(direction_in, sun_direction, normal, original_normal);

				float weight = pdf_light * pdf_light / (pdf_light * pdf_light + scatter_pdf * scatter_pdf);

				vec3 scatter_bsdf = bsdfs[mtl]->eval(direction_in, sun_direction, normal, original_normal, mats[mtl].Kd);

				L += scatter_bsdf * weight / pdf_light;
			}
		}
	...
	vec3 __fastcall sibenik_tracing_directional_lighting_NEE_scatter_original_2(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		//vec3 T_scatter(1.0f);
		vec3 T(1.0f);
		...
		for (int i = 0; i < 60; ++i)
		{
			...
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
				...
				if (isScatter2(distance))
				{
					...
					//L += T_scatter * sun_color * estimate_direct_lighting(n, new_ray.d, hit_point, rec.n, original_normal, mtl, Volume_type);

					L += T * sun_color * estimate_direct_lighting(n, new_ray.d, hit_point, rec.n, original_normal, mtl, Volume_type);
				...
				else
				{
					...
					//T_scatter *= Transmitance / atmos_pdf;
					...
					if (!bsdfs[mtl]->isSpecular())
					{
						...
						if (use_directional_light)
						{
								...
								//if(!hit_anything_range_alpha(n, directional_lighting, dist))
							{
								...
								if(isScatter2(light_distance, sigma_s_sun))
								{
									//cout << "scatter";
									vec3 hit_point(new_ray.o + new_ray.d * light_distance);
									...
									vec3 Transmitance(expf(-sigma_t_sun * light_distance));

									vec3 density = sigma_t_sun * Transmitance;

									float atmos_pdf = (density.x + density.y + density.z) / 3; //density.x;

									//T_scatter *= Transmitance * sigma_s / atmos_pdf;

									//T_scatter *= Transmitance * sigma_s_sun / atmos_pdf;

									T *= Transmitance * sigma_s_sun / atmos_pdf;
									...
									L += sun_color * sun_color * T * (pdf_scatter * pdf_scatter) / (pdf_scatter * pdf_scatter + pdf_light * pdf_light);
						...
						*/
						...
						L += T * sun_color * estimate_direct_lighting(n, new_ray.d, hit_point, rec.n, original_normal, mtl, mats[mtl].type);
	...
	vec3 __fastcall sibenik_tracing_directional_lighting_only_scatter_if_see_light(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		//vec3 T_scatter(1.0f);
		vec3 T(1.0f);
		...
		for (int i = 0; i < 60; ++i)
		{
			...
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
				...
				if (isScatter2(distance, sigma_s))
				{
					...
					L += T * estimate_direct_lighting(n, new_ray.d, hit_point, rec.n, original_normal, mtl, Volume_type);
	...
	float HG_pdf_sun(vec3& wi, vec3& wo, const float& g_sun)
	{
		...
		return Inv4Pi * (1.0 - g_sun * g_sun) / (denom * sqrt14(denom));
	}
	...
	vec3 VolumeTric_Rendering_Homogeneous_Attenuate_NEE(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 T(1.0f);
			...
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
				...
				else
				{
					...
					if (!bsdfs[mtl]->isSpecular())
					{
						...
						//NEE still have to consider scatter
						if (isScatter(length))
						{
							vec3 hit_point(new_ray.o + new_ray.d * length);
	...
	vec3 VolumeTric_Rendering_Homogeneous_Attenuate_NEE_Glass_Sphere(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 T(1.0f);
		...
		for (int i = 0; i < 60; ++i)
		{
			...
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
				...
				if (sph.intersect(new_ray, t))
				{
					if (t < rec.t)
					{
						rec.t = t;
						rec.n = (new_ray.o + new_ray.d * t - sph.p).norm();
						hit_sphere = true;
						mtl = mats.size() - 1;
					}
				}
	...
	vec3 VolumeTric_Rendering_Homogeneous_Attenuate_NEE_Glass_Sphere_2(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 T(1.0f);
		...
		for (int i = 0; i < 60; ++i)
		{
			...
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
				...
				if (isScatter2(distance))
				{
					...
					//vec3 density = sigma_a * Transmitance;
				...
				else
				{
					...
					if (!bsdfs[mtl]->isSpecular())
					{
						...
						if (isScatter(length))
						{
							...
							vec3 Transmitance(expf(-sigma_a * length));
	...
	vec3 VolumeTric_Rendering_Homogeneous_Attenuate_NEE_Glass_Sphere_2_NEE(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 T(1.0f);
			...
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
				...
				if (isScatter2(distance))
				{
					...
					if (!hit_anything_range_alpha(n, light_ray, length))
					{
						...
						float pdf_light = (length * length) / (Ls.area[li] * cos_light) * Ls.pdf[li];
						float pdf_scatter = HG_pdf(-new_ray.d, light_direction);

						float weight = pdf_light / (pdf_light * pdf_light + pdf_scatter * pdf_scatter);

						L += T * Ls.Ke[li] * weight;

						//L += T * expf(-sigma_a * length) * Ls.Ke[li] * weight;
					}
	...
	vec3 VolumeTric_Rendering_Homogeneous_Attenuate_NEE_Glass_Sphere_2_NEE_fog(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 T(1.0f);
	...
	vec3 VolumeTric_Rendering_Homogeneous_Attenuate_NEE_Glass_Sphere_Direct_Lighting(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 T(1.0f);
		...
		for (int i = 0; i < 60; ++i)
		{
			...
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
				...
				if (isScatter2(distance))
				{
					...
					//vec3 light_direction = vec3(0.1, 0.9, 0.0);

					light_direction = light_direction.norm();

					//check NEE for scatter
					Ray light_ray(hit_point, light_direction);
					...
					{
						vec3 normal = (0, -1, 0);
						float cos_light = abs(-light_direction.dot(normal));

						float pdf_light = (length * length) / (4 * cos_light) * 0.5;

						float pdf_sample = HG_pdf(new_ray.d, light_direction);

						float weight = (pdf_light) / (pdf_sample * pdf_sample + pdf_light * pdf_light);

						L += T * expf(-sigma_a * length) * weight;
					}
			...
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
				...
				if (isScatter2(distance))
				{
						...
						//if(!hit_anything_range(n, light_ray))
					{
						...
						L += T * 20;// *2 * expf(-sigma_a * length) * weight;
					}
	...
	*/
	vec3 god_ray_pbrt(node*& n, Ray& r)
	{
		...
		vec3 sun_direction = original_sun_direction;
		for (int i = 0; i < 60; ++i)
		{
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
			...
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
				...
				if (scatter_distance < rec.t)
				{
					...
					if (!hit_anything_range_alpha(n, sun_ray, d))
					{
						...
						//start virtual distance
						float th;

						n->box.hit_th(sun_ray, th);

						vec3 virtual_hit_point = sun_ray.o + sun_ray.d * th;

						float virtual_length = (virtual_hit_point - hit_point).length();

						float sun_distance_weight = expf(-sigma_a_sun * virtual_length);
						//end virtual distance
						L += T * sun_color_god_ray * weight * sun_distance_weight;
					}
	...
	/*
	vec3 Subsurface_Scattering(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 T(1.0f);
		...
		for (int i = 0; i < 60; ++i)
		{
			...
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
				...
				if (mats[mtl].type == SubSurface_Scattering_type)
				{
					//comming in
					if (cos_incident < 0.0f)
					{
						vec3 new_hit_point = new_ray.o + new_ray.d * rec.t * 1.0002f;

						//vec3 in_volume_direction = new_ray.d;

						Ray in_volume_ray(new_hit_point, new_ray.d);

						HitRecord new_rec;
						vec3 new_color;
						float new_alpha;
						if (hit_color_alpha(n, in_volume_ray, new_rec, &new_color, new_alpha))
						{
							int new_mtl = trs[new_rec.ti].mtl;

							vec3 hit_point_inside = in_volume_ray.o + in_volume_ray.d * new_rec.t;

							float length = (hit_point_inside - new_hit_point).length();

							//float sample_distance = mats[new_mtl].neg_inv_density * log(randf());

							float sample_distance = mats[new_mtl].neg_inv_density * log(randf());

							if (sample_distance <= length)
							{
								vec3 scatter_direction = Uniform_Sphere_Sampling(1.0f).norm();

								T *= expf(-sample_distance);

								new_ray = Ray(new_ray.o + new_ray.d * sample_distance, scatter_direction);

								continue;
							}
							else
							{
								vec3 new_point_out_side = in_volume_ray.o + in_volume_ray.d * new_rec.t * 1.0002f;

								new_ray = Ray(new_point_out_side, new_ray.d);
					...
					{
						vec3 direction = rec.t * new_ray.d;

						float length = direction.length();

						int new_mtl = trs[rec.ti].mtl;
						...
						{
							vec3 new_hit_point(new_ray.o + new_ray.d * rec.t * 1.0002f);
							new_ray = Ray(new_hit_point, new_ray.d);
							continue;
						}
					}
				}
	...
	*/
	...
	//s 20 a 0.5 
	const float subsurface_sigma_s = 20.0f;//0.02
	const float subsurface_sigma_a = 0.0005f;//0.005
	const float subsurface_sigma_t = subsurface_sigma_a + subsurface_sigma_s;

	vec3 Subsurface_Scattering_2(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 T(1.0f);
		...
		//for (int i = 0; i < mats.size(); ++i)
		//	if (mats[i].type == SubSurface_Scattering_type)
		//		cout << mats[i].type;    //SubSurface_Scattering_type
			...
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
				...
				if (mats[mtl].type == SubSurface_Scattering_type)
				{
					...
					if (cos_incident < 0.0f)
					{
						...
						HitRecord in_volume_rec;
						vec3 in_volume_color;
						float in_volume_alpha;
						if (hit_color_alpha(n, in_volume_ray, in_volume_rec, &in_volume_color, in_volume_alpha))
						{
							int new_mtl = trs[in_volume_rec.ti].mtl;

							vec3 hit_point_inside = in_volume_ray.o + in_volume_ray.d * in_volume_rec.t;

							float length = (hit_point_inside - new_hit_point).length();

							float sample_distance = -log(randf()) / subsurface_sigma_s;

							if (sample_distance < length)
							{
								//cout << "scatter\n";
								vec3 scatter_direction = Uniform_Sphere_Sampling().norm();

								T *= expf(subsurface_sigma_a * -sample_distance);

								new_ray = Ray(in_volume_ray.o + in_volume_ray.d * sample_distance, scatter_direction);

								continue;
							}
							else
							{
								T *= expf(subsurface_sigma_a * -length);
								vec3 new_point_out_side = in_volume_ray.o + in_volume_ray.d * in_volume_rec.t * 1.0002f;
								new_ray = Ray(new_point_out_side, new_ray.d);
								continue;
							}
						}
				...
				//{
					...
					else
					{
						...
						if (mats[mtl].type == Glass_Beer_Law_type)
						{
							//cout << "A";
							b = bsdfs[mtl]->sample_volume(new_ray.d, rec.n, original_normal, rec.t, sampling, isReflect, color);
						}
						else
							b = bsdfs[mtl]->sample(new_ray.d, rec.n, original_normal, sampling, isReflect, color);
	...
	vec3 Subsurface_Scattering_2_fix(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 T(1.0f);
		...
		for (int i = 0; i < 60; ++i)
		{
			...
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
				...
				if (mats[mtl].type == SubSurface_Scattering_type)
				{
					...
					//cout << "SubSurface\n";
					//comming in
					if (prev_type != mats[mtl].type)
					{
						//if (randf() < 0.9f)
							new_ray = Ray(new_ray.o + new_ray.d * rec.t * 1.0002f, new_ray.d);
						//else
						//	new_ray = Ray(new_ray.o + rec.n * 0.002f, Reflect(new_ray.d, rec.n));
					...
					else
					{
						...
						if (sample_distance <= length)
						{
							//return vec3(1);
							//cout << "a";
							vec3 scatter_direction = Uniform_Hemisphere_Sampling().norm();
						...
						{
							//cout << "b";
							//T *= expf(subsurface_sigma_a * -rec.t);
							T *= expf(subsurface_sigma_a * -length);
							vec3 new_hit_point(new_ray.o + new_ray.d * rec.t * 1.0002f);
							new_ray = Ray(new_hit_point, new_ray.d);
							//continue;
				...
				else
				{
					...
					else
					{
						...
						vec3 new_hit_point = new_ray.o + new_ray.d * rec.t + rec.n * 0.02f;
	...
	vec3 Subsurface_Scattering_2_fix_Reflection(node*& n, Ray& r)
	{
		...
		for (int i = 0; i < 60; ++i)
		{
			...
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
				...
				if (mats[mtl].type == SubSurface_Scattering_type)
				{
					...
					if (randf() <= F)
					{
						new_ray = Ray(new_ray.o + new_ray.d * rec.t + rec.n * 0.002f, Reflect(new_ray.d, rec.n).norm());	
						float inv_F = 1.0f / F;
						//T *= inv_F;
						T *= mats[mtl].Kd;
						continue;
					...
					{
						//new_ray = Ray(new_ray.o + new_ray.d * rec.t - rec.n * 0.002f, Refract(new_ray.d, rec.n, eta, cos_i, cos_t).norm());
						//new_ray = Ray(new_ray.o + rec.t * new_ray.d - rec.n * 0.002f, (eta * new_ray.d + (eta * abs_cos_i - cos_t) * rec.n).norm());
						
						//float inv_1F = 1.0f / (1.0 - F);
						//T *= inv_1F;
						T *= mats[mtl].Kd;
						...
						{
							vec3 scatter_direction = Uniform_Sphere_Sampling().norm();
													//Uniform_Hemisphere_Sampling().norm();
							//onb local(new_ray.d);

							//scatter_direction = local.u * scatter_direction.x + local.v * scatter_direction.y + local.w * scatter_direction.z;
						...
						else
						{
							//T *= expf(subsurface_sigma_a * -rec.t);
							T *= expf(subsurface_sigma_a * -length);
							vec3 new_hit_point(new_ray.o + new_ray.d * rec.t * 1.0002f);//- rec.n * 0.0002f);
							new_ray = Ray(new_hit_point, new_ray.d);
							continue;
						}
					}
	...
	vec3 Subsurface_Scattering_2_brute_force(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 T(1.0f);
					...
					if (cos_incident < 0.0f)
					{
						...
						if (hit_color_alpha(n, in_volume_ray, in_volume_rec, &in_volume_color, in_volume_alpha))
						{
							...
							if (sample_distance < length)
							{
								...
								vec3 Transmit = expf(subsurface_sigma_a * -sample_distance);

								vec3 pdf = subsurface_sigma_t * Transmit;

								vec3 f_sss = subsurface_sigma_s * Transmit;

								float pdf_balance = (pdf[0] + pdf[1] + pdf[2]) / 3.0;
								if (pdf_balance == 0)
									pdf_balance = 1.0;

								//float inv_pdf_balance = 1.0f / pdf_balance;
	...
	vec3 Subsurface_Scattering_2_brute_force_original(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 T(1.0f);
				...
				if (mats[mtl].type == SubSurface_Scattering_type)
				{
					...
					if (cos_incident < 0.0f)
					{
						...
						if (hit_color_alpha(n, in_volume_ray, in_volume_rec, &in_volume_color, in_volume_alpha))
						{
							...
							float sample_distance = -log(randf()) / subsurface_sigma_t;

							if (sample_distance + 1e-6 < length)
							{
								//cout << "scatter\n";
								vec3 scatter_direction = Uniform_Sphere_Sampling().norm();

								vec3 Transmit = expf(subsurface_sigma_t * -sample_distance);
	...
	vec3 Subsurface_Scattering_2_brute_force_original_modify(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 T(1.0f);
				...
				if (mats[mtl].type == SubSurface_Scattering_type)
				{
					...
					if (cos_incident < 0.0f)
					{
						...
						if (hit_color_alpha(n, in_volume_ray, in_volume_rec, &in_volume_color, in_volume_alpha))
						{
							...
							{
								//T *= expf(subsurface_sigma_a * -length);
								vec3 Transmit = expf(subsurface_sigma_s * -sample_distance);
	...
	vec3 Volume_Sample(Ray& r, float& max_t, bool& is_scatter, Ray& new_ray, float& pdf_balance)
	{
		float distance = -logf(randf()) / sigma_t;

		if (distance < max_t - 1e-6)
		{
			vec3 scattering_pos = r.o + r.d * distance;

			vec3 scatter_direction;

			float scatter_pdf = HG_Sample(-r.d, scatter_direction);

			new_ray = Ray(scattering_pos, scatter_direction);

			new_ray.in_volume = true;

			is_scatter = true;

			vec3 transmit = expf(-sigma_t * distance);
			vec3 pdf = sigma_t * transmit;
			vec3 f_sss = sigma_s * transmit;

			pdf_balance = (pdf[0] + pdf[1] + pdf[2]) / 3.0;
			if (pdf_balance == 0)
				pdf_balance = 1.0;
		...
		{
			vec3 transmit = expf(-sigma_t * max_t);
			vec3 pdf = transmit;
			vec3 f_sss = transmit;

			is_scatter = false;

			new_ray.in_volume = false;

			pdf_balance = (pdf[0] + pdf[1] + pdf[2]) / 3.0;
			if (pdf_balance == 0)
				pdf_balance = 1.0;

			return f_sss * (1.0f / pdf_balance);
		}
	}

	vec3 __fastcall Radiance_Volume(node*& n, Ray& r)
	{
		Ray new_ray(r);
		...
		for (int i = 0; i < 60; ++i)
		{
			...
			if (new_ray.in_volume)
			{
				if (hit_color_alpha(n, new_ray, rec, &color, alpha))
				{
					...
					if (mats[mtl].type == SubSurface_Scattering_type)
					{
						...
						vec3 weight = Volume_Sample(new_ray, max_t, is_scatter, scatter_ray, pdf_scatter);
						...
						{
							vec3 hit_point = (new_ray.o + new_ray.d * rec.t * 1.00002f);
			...
			else
			{
				if (hit_color_alpha(n, new_ray, rec, &color, alpha))
				{
					...
					if (mats[mtl].type == SubSurface_Scattering_type)
					{
						Ray volume_ray(new_ray.o + new_ray.d * rec.t * 1.0002f, new_ray.d);

						vec3 in_color;
						float in_alpha = 1.0f;
						HitRecord in_rec;

						if (hit_color_alpha(n, volume_ray, in_rec, &in_color, in_alpha))
						{
							specular = false;

							float max_t = in_rec.t;
							...
							vec3 weight = Volume_Sample(volume_ray, max_t, is_scatter, scatter_ray, pdf_scatter);
	...
	vec3 __fastcall Radiance_Volume_2(node*& n, Ray& r)
	{
		Ray new_ray(r);
		...
		for (int i = 0; i < 60; ++i)
		{
			...
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
				...
				if (mats[mtl].type = SubSurface_Scattering_type)
				{
					//cout << mats[mtl].type << "\n";
					//cout << "!";
					if (prev_type == SubSurface_Scattering_type)
					{
						float scatter_distance = -log(randf()) / sigma_t;
						...
						if (scatter_distance < distance)
						{
							...
							HG_Sample(-new_ray.d, scatter_direction);

							Ray scatter_ray(scatter_point, scatter_direction);

							vec3 transmit = expf(-sigma_t * scatter_distance);
							vec3 pdf = sigma_t * transmit;
							vec3 f_sss = sigma_s * transmit;

							float pdf_balance = (pdf[0] + pdf[1] + pdf[2]) * 0.3333f;
							if (pdf_balance == 0)
								pdf_balance = 1.0;

							vec3 weight = f_sss * (1.0f / pdf_balance);

							T *= weight;

							prev_type = SubSurface_Scattering_type;
						...
						else
						{
							...
							Ray volume_ray(hit_point, new_ray.d);

							vec3 transmit = expf(-sigma_t * rec.t);
							vec3 pdf = transmit;
							vec3 f_sss = transmit;
					...
					{
						vec3 interect_point(new_ray.o + new_ray.d * rec.t * 1.0002f);

						Ray in_volume_ray(interect_point, new_ray.d);
						...
						if (hit_color_alpha(n, in_volume_ray, in_rec, &in_color, in_alpha))
						{
							float scatter_distance = -log(randf()) / sigma_t;

							float distance = (in_rec.t * in_volume_ray.d).length();

							if (scatter_distance < distance)//in_rec.t)
							{
								vec3 scatter_point(in_volume_ray.o + in_volume_ray.d * scatter_distance);

								vec3 scatter_direction;

								HG_Sample(-in_volume_ray.d, scatter_direction);
							...
							{
								vec3 hit_point(in_volume_ray.o + in_volume_ray.d * in_rec.t * 1.0002f);

								Ray volume_ray(hit_point, in_volume_ray.d);

								vec3 transmit = expf(-sigma_t * in_rec.t);
								vec3 pdf = transmit;
								vec3 f_sss = transmit;
	...
	vec3 __fastcall sibenik_tracing_artificial_light_normal_Ke_SubSurface(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 T(1.0f);
		HitRecord rec;
		...
		{		
			...
			{		
				...
				if (mats[mtl].type == SubSurface_Scattering_type)
				{
					float distance = -log(randf()) / sigma_t;

					if (distance < rec.t - 1e-6)
					{
						vec3 hit_point(new_ray.o + new_ray.d * distance);

						vec3 direction_out;
						float scatter_pdf = HG_Sample(-new_ray.d, direction_out);
						...
						float pdf_balance = (pdf.x + pdf.y + pdf.z) * 0.3333f;

						if (pdf_balance == 0)
							pdf_balance = 1.0;

						float inv_pdf_blance = 1.0f / pdf_balance;
					...
					else
					{
						...
						new_ray = Ray(new_ray.o + new_ray.d * rec.t * 1.00002f, new_ray.d);
