...
const float sigma_s_ = 0.009;//0.05
...
struct Scene
{
	...
	//SMALLVPT
	float sampleSegment(float& epsilon, float& sigma, float& smax)
	{
	...
	{
		double s=2.0*e1-1.0, f = (1.0-g*g)/(1.0+g*s), cost = 0.5*(1.0/g)*(1.0+g*g-f*f), sint = sqrt14(1.0-cost*cost);
		//double s=2.0*e1-1.0, f = (1.0-g*g)/(1.0+g*s), cost = 0.5*(1.0/g)*(1.0+g*g-f*f), sint = sqrt(1.0-cost*cost);
		return vec3(cos(2.0 * pi * e2) * sint, sin(2.0 * pi * e2) * sint, cost);
	}

	float scatter(const Ray &r, vec3& scatter_direction, float& distance, float &sample_distance)
	{
		float r1 = randf();
		//float distance = tout - tin;
		sample_distance = sampleSegment(r1, sigma_s, distance);
	
		//Vec dir = sampleSphere(XORShift::frand(), XORShift::frand()); //Sample a direction ~ uniform phase function
		
		float e1 = randf();
		float e2 = randf();
		vec3 dir = sampleHG(g, e1, e2); //Sample a direction ~ Henyey-Greenstein's phase function
	
		onb coord(r.d);
		dir = coord.u * dir.x + coord.v*dir.y + r.d * dir.z;
		
		scatter_direction = dir;
		
		return (1.0 - exp(-sigma_s * distance));
	}
	...
	vec3 VolumeTric_Rendering_Homogeneous_2(node*& n, Ray& r)
	{
		...
		for (int i = 0; i < 60; ++i)
		{
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
				...
				float ms = scatter(new_ray, scatter_direction, distance, sample_distance);
				...
				{
					//cout << "Yes\n";
					vec3 hit_point(new_ray.o + new_ray.d * sample_distance);
					...
					float s;
					//vec3 wi = new_ray.d;
					float ps = scatter(new_ray, direction_out, rec.t, s);
	...
	//Volume_Smallvpt
	vec3 VolumeTric_Rendering_Homogeneous_Small_vpt(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 T(1.0f);
