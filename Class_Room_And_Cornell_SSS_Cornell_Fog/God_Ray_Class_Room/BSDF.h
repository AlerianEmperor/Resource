...
public:
	...
	//virtual bool sample_bssrdf(const vec3& direction_in, vec3& n, vec3& original_n, const vec3& direction_out, vec3& out_n, Sampling_Struct& sampling, bool& isReflect, vec3& color) const;
	virtual bool sample_volume(const vec3& direction_in, vec3& n, vec3& original_n, float& t, Sampling_Struct& sampling, bool& isReflect, vec3& color) const
	{
		return true;
	}
...
static vec3 cosine_hemisphere_Sampling_with_n(const vec3& n)
{
	...
	{
		local_u = vec3(0.0f, -1.0f, 0.0f);
		local_v = vec3(-1.0f, 0.0f, 0.0f);
	}
	else
	{
		const float a = 1.0f / (1.0f + n.z);
		const float b = -n.x * n.y * a;
		local_u = vec3(1.0f - n.x * n.x * a, b, -n.x);
		local_v = vec3(b, 1.0f - n.y * n.y * a, -n.y);
	}


	vec3 direction(local_u * coord.x + local_v * coord.y + n * coord.z);

	return direction.norm();
}

vec3 cosine_hemipshere_Sampling_with_u_v_n(const vec3& n, vec3& u, vec3& v)
{
	float x1 = randf(), x2 = randf();
	float r = sqrt14(x1);
	float theta = 2.0f * pi * x2;

	float c, s;
	FTA::sincos(theta, &c, &s);

	vec3 coord(r * c, r * s, sqrt14(1.0f - x1));
	...
	vec3 direction(u * coord.x + v * coord.y + n * coord.z);
...
//----------Oren-nayar-------
class Oren_nayar : public BSDF_Sampling
{
public:
	float A;
	float B;
	float roughness;
	Oren_nayar(float roughness_ = 0.9) : roughness(roughness_)
	{
		A = 1.0f - 0.5f * roughness * roughness / (roughness * roughness + 0.33f);
		B = 0.45f * roughness * roughness / (roughness * roughness + 0.09f);
	}
	bool sample(const vec3& direction_in, vec3& n, vec3& original_n, Sampling_Struct& sampling, bool& isReflect, vec3& color) const
	{
		vec3 u, v;

		vec3 d = cosine_hemipshere_Sampling_with_u_v_n(n, u, v);
		...
		float cos_theta_i = -direction_in.dot(n);
		float cos_theta_o = cos_o;

		//cos(phi_o - phi_i) = cos_phi_o * cos_phi_i - sin_phi_i * sin_phi_o
		//we can compute cos by first compute phi_i and phi_o using acos
		//but it can be inaccurate so just stick to math 

		float cos_phi_i = -direction_in.dot(u);
		float cos_phi_o = d.dot(u);

		float sin_phi_i = sqrt14(1.0f - cos_phi_i * cos_phi_i);
		float sin_phi_o = sqrt14(1.0f - cos_phi_o * cos_phi_o);

		float cos_d = cos_phi_i * cos_phi_o + sin_phi_i * sin_phi_o;

		float sin_alpha, tan_beta;

		float abs_cos_theta_i = abs(cos_theta_i);
		float abs_cos_theta_o = abs(cos_theta_o);

		if (abs_cos_theta_o > abs_cos_theta_i)
		{
			sin_alpha = sqrt14(1.0f - cos_theta_i * cos_theta_i);
			tan_beta = sqrt14(1.0f - cos_theta_o * cos_theta_o) / cos_theta_o;
		}
		else
		{
			sin_alpha = sqrt14(1.0f - cos_theta_o * cos_theta_o);
			tan_beta = sqrt14(1.0f - cos_theta_i * cos_theta_i) / cos_theta_i;
		}

		//vec3 weight = color * (A + (B * maxf(0.0f, cos_d) * sin_alpha * tan_beta));
		//vec3 weight = color * cos_o * (A + (B * maxf(0.0f, cos_d) * sin_alpha * tan_beta));
		vec3 weight = color * cos_o * (A + B * cos_d * sin_alpha * tan_beta);
		float pdf = cos_o * ipi;
	...
	vec3 eval(const vec3& direction_in, const vec3& direction_out, const vec3& n, vec3& original_n, vec3& color) const
	{
		//recompute x coordinate to compute phi
		vec3 u;
		if (n.z < -0.9999999f) 
			u = vec3(0.0f, -1.0f, 0.0f);
		else
		...
		float cos_theta_i = -direction_in.dot(n);
		float cos_theta_o = direction_out.dot(n);

		float cos_phi_i = -direction_in.dot(u);
		float cos_phi_o = direction_out.dot(u);
		...
		//vec3 weight = color * cos_theta_o * (A + (B * maxf(0.0f, cos_d) * sin_alpha * tan_beta));
		vec3 weight = color * cos_theta_o * (A + B * cos_d * sin_alpha * tan_beta);
		return weight;
	}
	float pdf(const vec3& direction_in, const vec3& direction_out, const vec3& n, vec3& original_n) const
	{
...
public:
	...
	bool sample(const vec3& direction_in, vec3& n, vec3& original_n, Sampling_Struct& sampling, bool& isReflect, vec3& color) const
	{
		...
		if (randf() < F)
		{
			...
			vec3 direction_out = direction_in - 2.0f * cos_i * n;
			//sampling = Sampling_Struct(direction_out, Kd * vec3(1.0f), F, true);
			sampling = Sampling_Struct(direction_out, Kd, F, true);
			isReflect = true;
			return true;
		}
		else
		{
			...
			//sampling = Sampling_Struct(direction_out, Kd * vec3(1.0f), 1.0f - F, true);
			sampling = Sampling_Struct(direction_out, Kd, 1.0f - F, true);
			isReflect = false;
			return true;
		}
		return true;
...
public:
	...
	vec3 Kd = vec3(0.2, 0.4, 0.6);

	float inv_neg_density = -10;
	//This part for Beer Law
	//Not very useful
	//Based on real data 
	//https://chempics.files.wordpress.com/2020/06/combinedribenainstructions.pdf
	...
	vec3 ext = vec3(0.0897079, 0.0816842, 0.0861554);

	Glass_Beer_Law() {}
	Glass_Beer_Law(float ior_) : ior(ior_) {}
	Glass_Beer_Law(float ior_, float density_) : ior(ior_), density(density_) {}
	Glass_Beer_Law(float ior_, float density_, vec3 Kd_) : ior(ior_), density(density_), Kd(Kd_) { inv_neg_density = -1.0f / density; }
	...
	bool sample_volume(const vec3& direction_in, vec3& n, vec3& original_n, float& t, Sampling_Struct& sampling, bool& isReflect, vec3& color) const
	{
		...
		if (cos_i <=0.0f)
		{
			...
			vec3 Tint = Kd * vec3(expf(-extinction_coeficient.x), expf(-extinction_coeficient.y), expf(-extinction_coeficient.z));
			//cout << "s";
			weight = Tint;
...
class Glass_Realistic : public BSDF_Sampling
{
public:
	float ior = 1.5f;
	Glass_Realistic() {}
	Glass_Realistic(float ior_) : ior(ior_) {}

	bool sample(const vec3& direction_in, vec3& n, vec3& original_n, Sampling_Struct& sampling, bool& isReflect, vec3& color) const
	{
		double cos_i = direction_in.dot(n);
		const double ior = 1.5;
		double eta = cos_i >= 0.0 ? ior : 1.0 / ior;

		vec3 real_normal = n;//cos_i >= 0.0 ? -n : n;
		...
		else
		{
			...
			Refractive_direction = (eta * direction_in + (eta * abs_cos_i - sqrt14(c2)) * real_normal);
			double cos_t = abs(Refractive_direction.dot(n));
		...
		if (randf() < P)
		{
			sampling.direction = (direction_in - 2.0 * direction_in.dot(real_normal) * real_normal).norm();
			sampling.weight = R / P;
			sampling.pdf = P;
			sampling.isSpecular = true;
		}
		else
		{
			sampling.direction = (Refractive_direction).norm();
			sampling.weight = (1.0 - R) / (1.0 - P);
			sampling.pdf = (1.0f - P);
			sampling.isSpecular = true;
		}
		return true;
...
class Rough_Plastic_Diffuse_Yblien : public BSDF_Sampling
{
public:
	float ior;
	float roughness;
	...
	Rough_Plastic_Diffuse_Yblien() {}
	Rough_Plastic_Diffuse_Yblien(float ior_, float roughness_) : ior(ior_), roughness(roughness_) {}
	Rough_Plastic_Diffuse_Yblien(Microfacet_Distribution dist_, float ior_, float roughness_) : dist(dist_), ior(ior_), roughness(roughness_) {}
...
public:
	...
	float roughness = 0.1;
	float thickness = 1.0f;
	vec3 sigmaA = vec3(0.0f);//vec3(0.2f, 0.2f, 0.2f);
...
class Rough_Conductor_TungSten_Fix_Black_Speckled : public BSDF_Sampling
{
public:
	float roughness;
	ComplexIor ior;
	...
	Rough_Conductor_TungSten_Fix_Black_Speckled() {}
	Rough_Conductor_TungSten_Fix_Black_Speckled(float roughness_, ComplexIor ior_) : roughness(roughness_), ior(ior_) {}
	Rough_Conductor_TungSten_Fix_Black_Speckled(Microfacet_Distribution dist_, float roughness_, ComplexIor ior_) : dist(dist_), roughness(roughness_), ior(ior_) {}
	...
	float pdf(const vec3& direction_in, const vec3& direction_out, const vec3& n, vec3& original_n) const
	{
		...
		float sample_pdf = abs(pdf_h / (4.0f * -direction_in.dot(h)));
...
/*
float GTR1(const float& cos_hn, const float& a)
{
	if (a >= 1.0f)
		return ipi;
	float a2 = a * a;
	float t = 1.0f + (a2 - 1.0f) * cos_hn * cos_hn;

	return (a2 - 1.0f) / (pi * log2f(a2) * t);
}

float GTR2(const float& cos_hn, const float& a)
{
	float a2 = a * a;
	float t = 1.0f + (a2 - 1.0f) * cos_hn * cos_hn;

	return (a2 - 1.0f) / (pi * t * t);
}
*/
float SmithGGX(const float& cos_vn, const float& alpha_g)
{
	float a = alpha_g * alpha_g;
	float b = cos_vn * cos_vn;

	return 1.0f / (cos_vn + sqrt14(a + b - a * b));
}
...
float Fr(const float& cos_vn, const float& etaI, const float& etaT)
{
	float sin_t2 = Sqr(etaI / etaT);

	if (sin_t2 > 1.0f)
		return 1.0f;

	float cos_ln = sqrt14(1.0f - sin_t2);

	float eta = etaT / etaI;

	float r1 = (cos_vn - eta * cos_ln) / (cos_vn + eta * cos_ln);
	float r2 = (cos_ln - eta * cos_vn) / (cos_ln + eta * cos_vn);
...
vec3 Uniform_Hemisphere_Sampling()
{
	...
	float x = cosf(phi) * sin_theta;// *r;

	float y = sinf(phi) * sin_theta;// *r;
...
//similar to uniform_hemisphere
vec3 Uniform_Sphere_Sampling()
{
	float theta = 2.0f * pi * randf();

	//hemisphere
	//float u = randf() [0, 1]
	//float u = 2.0f * randf() - 1.0f [-1, 1]
	float u = 2.0f * randf() - 1.0f;

	float sqrt_1u = sqrt14(1.0f - u * u);

	float cos_theta, sin_theta;

	FTA::sincos(theta, &sin_theta, &cos_theta);

	return vec3(sqrt_1u * cos_theta, sqrt_1u * sin_theta, u);
}
/*
class Disney : public BSDF_Sampling
{
public:
	float subsurface = 0.6f;
	float transmission = 0.2f;

	float ior = 1.5f;
	float ior_air = 1.000273f;
	...
	bool sample(const vec3& direction_in, vec3& n, vec3& original_n, Sampling_Struct& sampling, bool& isReflect, vec3& color) const
	{
		...
		{
			float cos_i = original_n.dot(-direction_in);

			float F = Fr(cos_i, ior, ior_air);
			...
			if (randf() < F)
			{
				...
				const float cosThetaHalf = sqrt14((1.0f - r2) / (1.0f + (Sqr(a) - 1.0f)*r2));
				const float sinThetaHalf = sqrt14(max(0.0f, 1.0f - Sqr(cosThetaHalf)));
				const float sinPhiHalf = sinf(phiHalf);
				const float cosPhiHalf = cosf(phiHalf);

				onb local(n);

				vec3 half = local.u * (sinThetaHalf * cosPhiHalf) + local.v * (sinThetaHalf*sinPhiHalf) + n * cosThetaHalf;

				if (half.dot(n) < 0.0f)
				{
					half = -half;
					sampling.direction = Reflect(direction_in, half);
				}
				else
				{
					float eta = ior / ior_air;
					float cos_t;
					sampling.direction = Refract(direction_in, n, eta, cos_i, cos_t);
					sampling.pdf = (1.0f - F) * transmission;
		...
		else
		{
			...
			if (randf() < 0.5f)
			{
				...
				if (randf() < subsurface)
				{
					const vec3 d = Uniform_Sphere_Sampling();//uniform(rand);
	...
	float pdf(const vec3& direction_in, const vec3& direction_out, const vec3& n, vec3& original_n) const
	{
		//if (direction_out.dot(original_n) <= 0.0f)
		float cos_i = direction_in.dot(original_n);
		...
		{
			float bsdf_pdf = 0.0f;
			float brdf_pdf = i4pi * subsurface;

			return Lerp(bsdf_pdf, brdf_pdf, transmission);
		}
		else
		{
			float F = Fr(direction_in.dot(original_n), ior, ior_air);

			const float a = max(0.001f, roughness);

			vec3 h = (-direction_in + direction_out).norm();

			const float cos_theta_half = abs(h.dot(n));

			float pdf_half = GTR2(cos_theta_half, a) * cos_theta_half;

			float cos_hl = direction_out.dot(h);

			float pdf_spec = 0.25f * pdf_half / max(1e-6, cos_hl);

			float pdf_diff = abs(direction_out.dot(n)) * ipi * (1.0f - subsurface);

			float bsdf_pdf = pdf_spec * F;

			float brdf_pdf = Lerp(pdf_diff, pdf_spec, 0.5f);

			return Lerp(brdf_pdf, bsdf_pdf, transmission);
		}
	}
	bool isSpecular() const
	{
...
public:
	...
	bool sample(const vec3& direction_in, vec3& n, vec3& original_n, Sampling_Struct& sampling, bool& isReflect, vec3& color) const
	{
		...
		float cos_theta = sqrt14(maxf(0.0f, (1.0f - powf(a2, 1.0f - r0)) / (1.0f - a2)));
		float sin_theta = sqrt14(maxf(0.0f, 1.0f - cos_theta * cos_theta));
...
class Subsurface_Scattering : public BSDF_Sampling
{
public:
	virtual bool sample(const vec3& direction_in, vec3& n, vec3& original_n, Sampling_Struct& sampling, bool& isReflect, vec3& color) const
	{

	}
	virtual vec3 eval(const vec3& direction_in, const vec3& direction_out, const vec3& n, vec3& original_n, vec3& color) const
	{

	}
	virtual float pdf(const vec3& direction_in, const vec3& direction_out, const vec3& n, vec3& original_n) const
	{
