...
static float max_value(const float& a, const float& b)
{
	return a > b ? a : b;
}

static float min_value(const float& a, const float& b)
{
	return a < b ? a : b;
}
...
struct vec3
{
	...
	float maxc() const { return max_value(x, max_value(y, z)); }//float d = max(x, y); return max(d, z); }//{ return max(max(x, y), z); }//
	float minc() const { return min_value(x, min_value(y, z)); }//float d = min(x, y); return min(d, z); }//{ return min(min(x, y), z); }//
	...
	friend vec3 sqrt(const vec3& a) { return vec3(sqrt14(a.x), sqrt14(a.y), sqrt14(a.z)); }
