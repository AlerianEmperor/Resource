...
const float sigma_a_ = 0.0002;//0.02;//0.005;//0.02;//0.02//0.0006;//0.006;//0.006;//0.09
const float sigma_s_ = 0.27;//0.04;//0.09;//0.036;//0.036//0.006;//0.036;//0.009;//0.05
...
struct Scene
{
	...
	vec3 infinite_light_direction;// = (-1, -0.1, 0.1);
	vec3 original_sun_direction;// = vec3(1.0f, 0.2f, 0.2f);
			...
			if (top->box.hit_shadow_th(r, th))// && th >= 0.0f)
			{
				...
				else
				{
					...
					for (auto i = start; i < end; ++i)
					{
						...
						HitRecord rec;
						if (trs[i].hit_anything_alpha(r, rec) && rec.t * 1.00000024f < d)
						{
							if (!mats[mtl].alpha_material)
								return true;
							else
				...
				if (isScatter2(distance))
				{
					...
					if (!hit_anything_range_alpha(n, light_ray, length))
					{
						...
						if (!sph.intersect(light_ray, t))
						{
							...
							vec3 bsdf_eval = expf(-sigma_a * length);//bsdfs[mtl]->eval(new_ray.d, light_direction, rec.n, original_normal, color);
							float bsdf_pdf = 0.5f * i2pi;


							float mis = mis2(pdf_light, bsdf_pdf * cont_prob);


							L += T * bsdf_eval * mis / (pdf_light * length2);
						}
					}
	...
	vec3 __fastcall sibenik_tracing_artificial_light_normal_Ke_alpha_Glass_NEE_fog(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 T(1.0f);
		HitRecord rec;
	...
	vec3 VolumeTric_Rendering_Homogeneous_Attenuate_NEE_Glass_Sphere_Direct_Lighting_2(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 T(1.0f);
	...
	*/
	...
	vec3 Volume_Sample(Ray& r, float& max_t, bool& is_scatter, Ray& new_ray)
	{
			...
			if (new_ray.in_volume)
			{
				if (hit_color_alpha(n, new_ray, rec, &color, alpha))
				{
					...
					if (mats[mtl].type == SubSurface_Scattering_type)
					{
						...
						vec3 weight = color * Volume_Sample(new_ray, max_t, is_scatter, scatter_ray);
					...
					vec3 hit_point(new_ray.o + new_ray.d * rec.t + rec.n * 0.02f);
				...
				if (hit_color_alpha(n, new_ray, rec, &color, alpha))
				{
					...
					if (mats[mtl].type == SubSurface_Scattering_type)
					{
						...
						if (hit_color_alpha(n, volume_ray, in_rec, &in_color, in_alpha))
						{
							...
							vec3 weight = Volume_Sample(volume_ray, max_t, is_scatter, scatter_ray);
							...
							{
								vec3 hit_point = (new_ray.o + new_ray.d * rec.t * 1.002f);
					...
					vec3 new_hit_point = new_ray.o + new_ray.d * rec.t + rec.n * 0.002f;
