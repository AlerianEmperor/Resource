...
#include "SubSurface_Scattering_2.h"
...
void de_purple_de_black(string& file_name, string& new_file_name)
{
	ifstream ifs(file_name);
	...
	for (int i = 0; i < size; ++i)
	{
		...
		//if (color[i].y == 0 && color[i].x == 0 && color[i].z == 0)
		if (color[i].y <= 1.0 && color[i].x <= 1.0 && color[i].z <= 1.0)
		{
			int x_coord = i % Width;
			int y_coord = i / Width;

			int count_none_purple = 0;
			int count_none_black = 0;
			vec3 sum(0.0f);
			vec3 sum_black(0.0f);
			...
			if (count_none_purple >= 1)
				color[i] = sum / count_none_purple;
			if (count_none_black >= 2)
				color[i] = sum_black / count_none_purple;
		}
	}
...
void de_purple_de_black_de_infinity(string& file_name, string& new_file_name)
{
	ifstream ifs(file_name);
...
void de_purple_de_infinity(string& file_name, string& new_file_name)
{
	ifstream ifs(file_name);
...
int a = 0;
void de_purple_de_black_de_infinity_de_speckle(string& file_name, string& new_file_name)
{
	ifstream ifs(file_name);
	...
	for (int i = 0; i < size; ++i)
	{
		int x_coord = (i + a) % Width;
		int y_coord = (i + a) / Width;
		...
		if (color[i].y <= 1.0 && color[i].x <= 1.0 && color[i].z <= 1.0)
		{
			...
			if (count_none_black >= 2)
				color[i] = sum_black / count_none_black;
		}

		//depseckle
		int count_speckle = 0;

		count_speckle += color[i].x <= 0.1f;
		count_speckle += color[i].y <= 0.1f;
		count_speckle += color[i].z <= 0.1f;
		...
		if (count_speckle >= 2)
		{
			...
			for (int j = 0; j < 4; ++j)
			{
				...
				if (new_x >= 0 && new_x < Width && new_y >= 0 && new_y < Height)
				{
					...
					if (color[ind].x > 0.0f != color[i].x > 0.0f ||
						color[ind].y > 0.0f != color[i].y > 0.0f ||
						color[ind].z > 0.0f != color[i].z > 0.0f)
						++count_4;
					sum_speckle += color[ind];
				}
			}

			if (count_4 == 4)
				color[i] = sum_speckle * 0.25f;
		}
	}
...
void main_1()
{
	string file_name = "Cornell_Box_Fog_10240_v2.ppm";
	...
	//string new_file_name = file_name + "De_Purple_1_sum.ppm";
	
	//string new_file_name = file_name + "De_Purple_Black.ppm";
	...
	string new_file_name = file_name + "de_purple_de_black_de_infinity_de_speckle.ppm";

	de_purple_de_black_de_infinity_de_speckle(file_name, new_file_name);

}

vector<vec3> get_file(string& s)
{
	ifstream ifs(s);
...
void sum_up(string& s1, string& s2)
{
	vector<vec3> color1 = get_file(s1);
	vector<vec3> color2 = get_file(s2);
	...
	for (int i = 0; i < Width * Height; ++i)
	{
		...
		//color1[i] = 2.0f * color1[i] + color2[i];
		//color1[i] = color1[i] / 3.0f;

		color2[i] += 2.0f * color1[i];
		color2[i] = color2[i] / 3.0f;
	}

	//ofstream ofs("Cornell_Box_Fog_20480.ppm");

	ofstream ofs("ClassRoom_Fog_12298.ppm");
	ofs << "P3\n" << Width << " " << Height << "\n255\n";
	...
	for (int i = 0; i < color2.size(); ++i)
	{
		...
		ofs << color2[i].x << " " << color2[i].y << " " << color2[i].z << "\n";
	}
...
void main()
{
	//string s1 = "Cornell_Box_Fog_10240.ppmde_purple_de_black_de_infinity_de_speckle.ppm";
	//string s2 = "Cornell_Box_Fog_10240_v2.ppmde_purple_de_black_de_infinity_de_speckle.ppm";

	string s1 = "god_ray_Class_Room_Final_Test_Before_Render_fog_2_final_Real_Class_Room.ppm_8192_9.ppm";
	string s2 = "god_ray_Class_Room_4096.ppm_4096_9.ppm";
...
void main_2()
{
	...
	*/
		...
		for (int i = 0; i < Width; ++i)
		{
			...
			for (int s = 0; s < ns; s += step)
			{
				...
				for (int num = 0; num < step; ++num)
				{
					...
					//!!!!!!!!!!Ham nay dung cho subsurface Scattering Rat Rat Quan Trong
					//SubSurface_Scattering.h
					//vec3 current_color(denan(scn.Radiance_Volume(n, new_ray)));

					//!!!!!!!Ham nay danh cho cornelbox volumetric Rat Quan Trong
					//SubSurface Scattering_2.h
					vec3 current_color(denan(scn.VolumeTric_Rendering_Homogeneous_Attenuate_NEE_Glass_Sphere_2_NEE_fog(n, new_ray)));
					//vec3 current_color(scn.Volumetric_Rendering_2(n, new_ray));
	...
	ofstream ofs("Cornell_Box_Fog_10240.ppm");
	//ofstream ofs("Volume_2048_color.ppm");
	//ofstream ofs("Radiance_Yellow_Light_fix_hit_range_randf_sample_no_multi_jitter_trang.ppm");
