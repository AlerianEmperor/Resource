...
struct Scene
{
	vec3 const_light_direction = vec3(0.1f, 0.6f, -0.05f);
	Scene(const string& filename)
	{
		sigma_a = vec3(0.001764, 0.0032095, 0.0019617);
		sigma_s = vec3(0.31845, 0.31324, 0.30147);
		sigma_t = sigma_a + sigma_t;
		g = g_;

		const_light_direction.normalize();
	...
	*/
		...
		for (int i = 0; i < 60; ++i)
		{
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
				...
				if (isScatter(distance))
				{
					...
					vec3 Transmitance(expf(-sigma_t.x * distance), expf(-sigma_t.y * distance), expf(-sigma_t.z * distance));
		...
		for (int i = 0; i < 60; ++i)
		{
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
				...
				if (mats[mtl].isLight || mats[mtl].type == Light_Diffuse_type)
				{
					...
					T *= vec3(expf(-sigma_t.x * rec.t), expf(-sigma_t.y * rec.t), expf(-sigma_t.z * rec.t));
				...
				if (isScatter(distance))
				{
					...
					if (!hit_anything_range(n, shadow_ray))
					{
						...
						vec3 attenuation = vec3(expf(-sigma_t.x * length), expf(-sigma_t.y * length), expf(-sigma_t.z * length));
				...
				else
				{
					...
					if (!bsdfs[mtl]->isSpecular())
					{
						...
						if (cos_mtl > 0.0f && !hit_anything_range(n, light_ray))
						{
							...
							T *= vec3(expf(-sigma_t.x * length), expf(-sigma_t.y * length), expf(-sigma_t.z * length));
	...
	vec3 VolumeTric_Rendering_Homogeneous_Directional(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 T(1.0f);
		...
		for (int i = 0; i < 60; ++i)
		{
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
				...
				if (isScatter(distance))
				{
					...
					vec3 light_direction = const_light_direction;//(light_point - hit_point);
					...
					if(hit_anything_range_alpha(n, shadow_ray, length))
					{
						...
						float pdf_light = 0.2f;//(length * length) / (Ls.area[li] * cos_light) * Ls.pdf[li];
				...
				else
				{
					...
					if (!bsdfs[mtl]->isSpecular())
					{
						...
						vec3 light_direction = const_light_direction;
