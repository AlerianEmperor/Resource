...
struct Scene
{
	...
	*/
	...
	//back 400
	vec3 picture_light_position_2 = vec3(2047.626465f, 2346.672607f, -11423.712891f);
	...
	vec3 __fastcall sibenik_tracing_directional_lighting_circle_blur_final_version(node*& n, Ray& r)
	{
		...
		for (int i = 0; i < 60; ++i)
		{
			...
			if (hit_color_alpha(n, new_ray, rec, &color, Tr))
			{
				...
				float rx, rz;
				random_point_on_circle(pic_light_radius, rx, rz);
				vec3 picture_light_point = picture_light_position_2 + vec3(rx, 0, rz);
				...
				float cos_view = -cylinder_light_direction.dot(vec3(0.0f, -1.0f, 0.0f));
				float cos_mtl = cylinder_light_direction.dot(rec.n);
				...
				if (cylinder_length <= 2600.0f && cos_mtl >= 0.0f && cos_view >= 0.2f && hit_point.y <= 2246.0f && !hit_anything_range_alpha(n, cylinder_light_ray, cylinder_length))
				{
					...
					float pdf_light = cylinder_length2 / (cylinder_light_area * cos_mtl);
					...
					//L += T * 4 * vec3(1.0f, 0.513f, 0.0f) * bsdf_eval * cos_mtl;

					//L += T * 4 * vec3(1.0f, 0.513f, 0.0f) * bsdf_eval * cos_mtl;// *mis;// / pdf_light;

					float real_length = cylinder_length;// *0.002f;

					float inv_real_length2 = 1.0f / (real_length * real_length);

					L += T * 400000.0f * vec3(1.0f, 0.513f, 0.0f) * bsdf_eval * mis / pdf_light;
	...
	vec3 __fastcall cone_light(node*& n, Ray& r)
	{
		...
		for (int i = 0; i < 60; ++i)
		{
			...
			if (hit_color_alpha(n, new_ray, rec, &color, Tr))
			{
				...
				if (cylinder_length <= 2200.0f && hit_point.y <= 2460.0f)////3000.0f)
				{
					//cout << "Hit Cylinder\n";
					//float pdf_light = pdf_light_radius;

					float real_length = cylinder_length * 0.01f;

					float real_length2 = (real_length * real_length);

					float pdf_light_cylinder = real_length2 / (cylinder_light_area * cos_view);

					vec3 bsdf_eval = bsdfs[mtl]->eval(new_ray.d, cylinder_light_direction, rec.n, original_normal, color);
					float bsdf_pdf = bsdfs[mtl]->pdf(new_ray.d, cylinder_light_direction, rec.n, original_normal);


					float mis = mis2(pdf_light_cylinder, bsdf_pdf * cont_prob);
					...
					L += T * 4 * vec3(1.0f, 0.513f, 0.0f) * bsdf_eval * mis / pdf_light_cylinder;// *inv_real_length2;// *cos_mtl;
