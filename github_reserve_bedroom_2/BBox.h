...
static float maxf2(const float& a, const float& b)
{
	return a > b ? a : b;
}

static float minf2(const float& a, const float& b)
{
	return a < b ? a : b;
}
/*
...
struct BBox
{
	...
	bool __fastcall hit_axis_tl(const Ray& r, float& tl) const
	{
		...
		for (int i = 2; i >= 0; --i)
		{
			...
			txmax[i] = (bbox[r.sign[i]][i] + r.roinvd[i]) * r.invd[i];
		}

		float th = minf2(txmax[2], minf2(txmax[1], txmax[0]));


		if (th < 0.0f)
			return false;

		/*float txmin = bbox[!r.sign[0]].x * r.invd.x + r.roinvd.x;
		float tymin = bbox[!r.sign[1]].y * r.invd.y + r.roinvd.y;
		float tzmin = bbox[!r.sign[2]].z * r.invd.z + r.roinvd.z;*/

		float txmin = (bbox[!r.sign[0]].x + r.roinvd.x) * r.invd.x;
		float tymin = (bbox[!r.sign[1]].y + r.roinvd.y) * r.invd.y;
		float tzmin = (bbox[!r.sign[2]].z + r.roinvd.z) * r.invd.z;

		tl = maxf2(txmin, maxf2(tymin, tzmin));
	...
	bool __fastcall hit_shadow_th(const Ray& r, float& th) const
	{
		...
		float txmin = (bbox[!r.sign[0]].x + r.roinvd.x) * r.invd.x;
		float txmax = (bbox[r.sign[0]].x + r.roinvd.x) * r.invd.x;

		float tymin = (bbox[!r.sign[1]].y + r.roinvd.y) * r.invd.y;
		float tymax = (bbox[r.sign[1]].y + r.roinvd.y) * r.invd.y;

		float tzmin = (bbox[!r.sign[2]].z + r.roinvd.z) * r.invd.z;
		float tzmax = (bbox[r.sign[2]].z + r.roinvd.z) * r.invd.z;

		float tl = maxf2(txmin, maxf2(tymin, tzmin));

		th = minf2(txmax, minf2(tymax, tzmax));
