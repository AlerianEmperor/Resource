...
struct Scene
{
	...
	{
		cylinder_light_vector = cylinder_light_vector.norm();
	...
	float circle_light_radius = 6.2f;

	float circle_light_area = 4.0f * pi * circle_light_radius * circle_light_radius;
	
	float pic_light_radius = 6.0f;

	float cylinder_light_area = 4.0f * pi * pic_light_radius * pic_light_radius;

	vec3 cylinder_light_vector = vec3(0.0f, -0.9f, -0.25f);

	//center light
	//1389.69 1144.71 - 11692.9
	//2672.95 2349.78 - 11672.9

	float const_cos_view = 0.6f;
	float const_vertical_distance = 9.5f;

	//X position
	//left
	//move left 10
	//float limit_x = 2052.427246f;
	//move left 20
	float limit_x = 2042.427246f;

	//Y position
	//up 10
	float limit_y = 2300.87532f;
	//up 100
	//float limit_y = 2390.87532f;
	//up 600
	...
	//up 1220
	//float limit_y = 3510.875732f;
	//up 1240
	//float limit_y = 3530.875732f;
	//up 1260
	//float limit_y = 3550.875732f;
	//up 1400
	//float limit_y = 3490.875732f;

	//Z position
	//back 80
	float limit_z = -11554.795898f;
	//back 280
	//float limit_z = -11354.795898f;
	//back 800
	//float limit_z = -10834.795898f;
	//back 750
	//float limit_z = -10884.795898f;
	//back 780
	//float limit_z = -10854.795898f;
	//back 700
	//float limit_z = -10934.795898f;

	//original
	//vec3 picture_light_position = vec3(2083.6, 2237.5, -11611.8);

	//back 300
	//vec3 picture_light_position = vec3(2083.6, 2237.5, -11311.8);

	//left 10 back 300
	//vec3 picture_light_position = vec3(2073.6, 2237.5, -11311.8);

	//left 20 back 300 up 200
	//vec3 picture_light_position = vec3(2063.6, 2437.5, -11311.8);

	//left 30 back 300 up 200
	//vec3 picture_light_position = vec3(2053.6, 2437.5, -11311.8);

	//left 40 back 300 up 200
	//vec3 picture_light_position = vec3(2043.6, 2437.5, -11311.8);

	//left 40 back 300 up 300
	//vec3 picture_light_position = vec3(2043.6, 2537.5, -11311.8);

	//left 38 back 300 up 300
	//vec3 picture_light_position = vec3(2045.6, 2537.5, -11311.8);

	//left 38 back 300 up 270
	//vec3 picture_light_position = vec3(2045.6, 2507.5, -11311.8);

	//left 36 back 300 up 270
	//vec3 picture_light_position = vec3(2047.6, 2507.5, -11311.8);

	//center light
	//1389.69 1144.71 - 11692.9
	//2672.95 2349.78 - 11672.9

	vec3 picture_light_position = vec3(2031.32f, 2507.5f, -11311.8f);

	//vec3 picture_light_position_2 = vec3(2047.626465f, 2346.672607f, -11623.712891f);

	//back 300
	//vec3 picture_light_position_2 = vec3(2047.626465f, 2346.672607f, -11323.712891f);

	//back 200
	//vec3 picture_light_position = vec3(2047.626465f, 2346.672607f, -11423.712891f);

	//back 600
	//vec3 picture_light_position_2 = vec3(2047.626465f, 2346.672607f, -11023.712891f);


	vec3 light_pos = //vec3(2063.422363, 2206.701904, -11416.070313);//vec3(2063.422363, 2406.701904, -11616.070313);
				...
				if (temp_tex.find(realname) == temp_tex.end())
				{
					...
					*/
					...
					if (n == 3)
					{
						...
						//	b = 6.0f;
						//if (realname == "impossible_stair.png")
						//	b = 1.4f;
	...
	*/
	...
	void ReadObj2(const string& p, const string& enviroment_path)
	{
		...
		vec3 maxp = vec3(INT_MIN, INT_MIN, INT_MIN), minp = vec3(INT_MAX, INT_MAX, INT_MAX);
		...
		while (f.getline(line, 1024))
		{
			...
			else if (strncmp(t, "f", 1) == 0 && add_face)
			{
				...
				for (int i = 0; i < faces.size(); ++i)
				{
					...
					if (material_name == "Rectangle2106816061")
					{
						//cout << "x";
						minp = min_vec(minp, vertex1);
						maxp = max_vec(maxp, vertex1);

						minp = min_vec(minp, vertex2);
						maxp = max_vec(maxp, vertex2);

						minp = min_vec(minp, vertex3);
						maxp = max_vec(maxp, vertex3);
					}
		...
		cout << minp.x << " " << minp.y << " " << minp.z << "\n";
		cout << maxp.x << " " << maxp.y << " " << maxp.z << "\n";
	...
	vec3 __fastcall sibenik_tracing_directional_lighting_circle_blur_final_version(node*& n, Ray& r)
	{
		...
		for (int i = 0; i < 60; ++i)
		{
			...
			if (hit_color_alpha(n, new_ray, rec, &color, Tr))
			{
				...
				random_point_on_circle(circle_light_radius, x, z);
				...
				float cos_mtl_circle_light = light_direction.dot(rec.n);
				

				if (cos_mtl_circle_light > 0.0f && length <= 1000 && hit_point.y < 2406.7f && !hit_anything_range_2(n, light_ray, length))
					...
					//if(!hit_anything_range_alpha(n, light_ray, length))
				{
					...
					float cos_view_circle_light = -light_direction.y;

					float pdf_light = length2 / (cylinder_light_area * cos_mtl_circle_light * cos_view_circle_light);
				...
				//vec3 picture_light_point = picture_light_position_2 + random_sphere_point; 
				...
				//vec3 picture_light_point = picture_light_position;

				vec3 cylinder_light_direction = (picture_light_point - hit_point);

				float cylinder_length2 = cylinder_light_direction.length2();

				float cylinder_length = sqrt14(cylinder_length2);

				cylinder_light_direction = cylinder_light_direction.norm();

				Ray cylinder_light_ray(hit_point, cylinder_light_direction);

				float cos_view = -cylinder_light_direction.dot(cylinder_light_vector);//-cylinder_light_direction.dot(vec3(0.0f, -1.0f, 0.0f));
				float cos_mtl = cylinder_light_direction.dot(rec.n);

				//if (cylinder_length <= 2600.0f && cos_view >= const_cos_view && cos_mtl >= 0.0f && hit_point.y <= 2420.0f)
				//if (cylinder_length <= 2600.0f && cos_mtl >= 0.0f && hit_point.y <= 2420.0f && !hit_anything_range_alpha(n, cylinder_light_ray, cylinder_length))
				if (cylinder_length <= 2000.0f && cos_mtl >= 0.0f && cos_view >= const_cos_view && hit_point.y <= 2306.0f && !hit_anything_range_alpha(n, cylinder_light_ray, cylinder_length))
				{

					//float pdf_light = pdf_light_radius;

					//float pdf_light = 1.0f; // pdf_light_cylinder;

					float pdf_light = cylinder_length2 / (cylinder_light_area * cos_mtl * cos_view);
					...
					//float real_length = cylinder_length;// *0.002f;

					float inv_real_length2 = 1.0f / cylinder_length2;//(real_length * real_length);

					L += T * 40000.0f * vec3(1.0f, 0.513f, 0.0f) * bsdf_eval * mis / pdf_light;
	...
	vec3 __fastcall sibenik_tracing_directional_lighting_circle_blur_final_version_limit_cone(node*& n, Ray& r)
	{
		vec3 L(0.0f);
		vec3 T(1.0f);
		HitRecord rec;
		...
		for (int i = 0; i < 60; ++i)
		{
			...
			if (hit_color_alpha(n, new_ray, rec, &color, Tr))
			{
				...
				float cos_view_circle_light = light_direction.y;//-light_direction.dot(vec3(0.0f, -1.0f, 0.0f));//-light_direction.y;

				if (cos_mtl_circle_light > 0.0f && cos_view_circle_light > 0.0f && length <= 1000.0f && hit_point.y <= 2798.2f && !hit_anything_range_2(n, light_ray, length))
					...
					//if(!hit_anything_range_alpha(n, light_ray, length))
				{
					...
					float pdf_light = length2 / (circle_light_area * cos_mtl_circle_light * cos_view_circle_light);
					...
					//float inv_length2 = 1.0f / length2;
					//L += T * 200.0f * vec3(1.0f, 1.0f, 0.2f) * bsdf_eval * mis * cos_mtl / pdf_light;// *100000 * inv_length2;// *mis / pdf_light;// * 0.1f
					
					L += T * 40.0f * vec3(1.0f, 0.27f, 0.0f) * bsdf_eval * mis * cos_mtl / pdf_light;
				...
				float cos_view = -cylinder_light_direction.dot(vec3(0.0f, -1.0f, 0.0f));//-cylinder_light_direction.dot(vec3(0.0f, -1.0f, 0.0f));
				float cos_mtl = cylinder_light_direction.dot(rec.n);

				float dist_to_Center = (picture_light_point - picture_light_position).length();

				float dist_to_rim = dist_to_Center + pic_light_radius;

				float cos_horizontal = -cylinder_light_direction.dot(vec3(1, 0, 0));
				
				cos_horizontal = abs(cos_horizontal);

				float distant_atenuse = dist_to_rim / cos_horizontal;

				float distant_vertical = sqrt14(distant_atenuse * distant_atenuse - dist_to_rim * dist_to_rim);

				//if (cylinder_length <= 2600.0f && cos_view >= const_cos_view && cos_mtl >= 0.0f && hit_point.y <= 2420.0f)
				//if (cylinder_length <= 2600.0f && cos_mtl >= 0.0f && hit_point.y <= 2420.0f && !hit_anything_range_alpha(n, cylinder_light_ray, cylinder_length))
				//if (distant_vertical >= const_vertical_distance && cylinder_length <= 2000.0f && cos_mtl > 0.0f && cos_view >= const_cos_view && hit_point.y <= 2506.0f && !hit_anything_range_alpha(n, cylinder_light_ray, cylinder_length))
				//if (cylinder_length <= 2000.0f && cos_mtl > 0.0f && cos_view >= const_cos_view && hit_point.y <= 2506.0f && !hit_anything_range_alpha(n, cylinder_light_ray, cylinder_length))
				
				
				//if(cos_view < const_cos_view && cos_view > const_cos_view - 0.05f)

				float blur = randf() * 0.16f;//(randf() <= 0.5f ? -1.0f : 1.0f) * 0.09f;

				//blur rim
				if (distant_vertical >= const_vertical_distance && cylinder_length <= 2000.0f && cos_mtl > 0.0f && cos_view >= const_cos_view - blur && hit_point.y <= 2506.0f && !hit_anything_range_alpha(n, cylinder_light_ray, cylinder_length))
				{
					...
					//L += T * 20000.0f * vec3(1.0f, 0.4f, 0.0f) * bsdf_eval * mis / pdf_light;
					
					L += T * 20000.0f * vec3(1.0f, 0.27f, 0.0f) * bsdf_eval * mis / pdf_light;
