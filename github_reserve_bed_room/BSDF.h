...
struct Sampling_Struct
{
	...
	//Sampling_Struct(Ray sampling_ray_, vec3 weight_, float pdf_, bool isSpecular_) : sampling_ray(sampling_ray_), weight(weight_), pdf(pdf_), isSpecular(isSpecular_) {}
	//Ray sampling_ray;
	vec3 direction;
...
float clampf(const float& v)
{
	return minf(maxf(0.0f, v), 1.0f);
}
...
public:
	...
	Rough_Plastic_Diffuse(float ior_, float roughness_, vec3 spec_color_) : ior(ior_), roughness(roughness_), spec_color(spec_color_) { eta = 1.0f / ior; }
...
public:
	...
	vec3 eval(const vec3& direction_in, const vec3& direction_out, const vec3& n, vec3& original_n, vec3& color)
	{
		...
		vec3 h = (direction_out - direction_in).norm();
...
vector<vec3> Sheen_Albedo;
int Sheen_w;
int Sheen_h;

//Charlie Sheen Production Frinedly BRDF 2017
class Cloth_Albedo_Scalling : public BSDF_Sampling
{
public:
	...
	vec3 Sheen = vec3(0.1f, 0.1f, 0.1f);
	vec3 ks = vec3(0.5f, 0.5f, 0.5f);

	float max_sheen = 0.1f;

	

	Cloth_Albedo_Scalling()
	{
		inv_roughness = 1.0f / roughness;
		float r = 1.0f - (1.0f - roughness) * (1.0f - roughness);//1.0f - roughness * roughness;
		a = Lerp(25.3245f, 21.5473f, r);
	...
	Cloth_Albedo_Scalling(float roughness_) : roughness(roughness_)
	{
		inv_roughness = 1.0f / roughness;
		float r = 1.0f - (1.0f - roughness) * (1.0f - roughness);
		a = Lerp(25.3245f, 21.5473f, r);
	...
	float Charlie_G(float& cos_i, float& cos_o)
	{
		...
		return 1.0f / ((1.0f + lambdaV + lambdaL) * (4.0f * abs(cos_i) * abs(cos_o)));
	}

	vec3 E(float& x)
	{
		int coordinate = roughness * Sheen_w + x;

		return Sheen_Albedo[coordinate];
	}
	...
	vec3 eval(const vec3& direction_in, const vec3& direction_out, const vec3& n, vec3& original_n, vec3& color)
	{
		...
		float F = Dielectric_Reflectance(eta, cos_i_h);
		...
		vec3 diff = color * (1.0f - F) * cos_o;

		vec3 spec = F * D * G;// *pi;

		//https://github.com/sebavan/glTF/tree/KHR_materials_sheen/extensions/2.0/Khronos/KHR_materials_sheen#theory-documentation-and-implementations
		vec3 e_i = E(cos_i);
		vec3 e_o = E(cos_o);

		vec3 Albedo_Scalling = min_vec(vec3(1.0f) - max_sheen * e_i, vec3(1.0f) - max_sheen * e_o);


		return Sheen * color + (diff + spec) * Albedo_Scalling;
	}
	float pdf(const vec3& direction_in, const vec3& direction_out, const vec3& n, vec3& original_n)
	{
		return i2pi;
...
public:
	...
	bool sample(const vec3& direction_in, vec3& n, vec3& original_n, Sampling_Struct& sampling, bool& isReflect, vec3& color)
	{
		...
		//{
			...
			float out = 0.25f * D * G / cos_o;

			float pdf = i2pi;

			sampling = Sampling_Struct(direction_out, color * out, pdf, false);
...
class Corona_Cloth : public BSDF_Sampling
{
public:
	float ior = 1.5f;
	float eta;
	...
	bool sample(const vec3& direction_in, vec3& n, vec3& original_n, Sampling_Struct& sampling, bool& isReflect, vec3& color)
	{
		...
		if (randf() < F)
		{
			...
			float dot1 = 1.0f;
			float dot2 = -direction_in.dot(direction_out);

			vec3 weight = color * (roughness + 1.0f) * i2pi * (F * powf(dot1, roughness) + (1.0f - F) * powf(dot2, roughness));

			sampling = Sampling_Struct(direction_out, weight, i2pi, true);
				//hit->rd*(fresnel*((exp + 1.0)*powf(dot1, exp)) / (2.0*M_PI) +
		...
		else
		{
			...
			vec3 reflectance_ray = Reflect(direction_in, n);

			float dot1 = direction_out.dot(reflectance_ray);
			float dot2 = -direction_in.dot(direction_out);

			vec3 weight = color * (roughness + 1.0f) * i2pi * (F * powf(dot1, roughness) + (1.0f - F) * powf(dot2, roughness));

			sampling = Sampling_Struct(direction_out, weight, i2pi, false);
...
float D_velvet_neubel(float& roughnees2, float& cot_2h, float& sin_4h)
{
	return ipi / (1.0f + 4.0f * roughnees2) * (1.0f + 4.0f * expf(-cot_2h / roughnees2) / sin_4h);
}

class Cloth_Diffuse : public BSDF_Sampling
{
public:
	float roughness = 0.1f;
	...
	float w_constant = w / ((1.0f + w) * (1.0f + w));

	vec3 subsurface_color = vec3(0.7f, 0.7f, 0.7f);

	vec3 Sheen_Color = vec3(0.9f, 0.9f, 0.9f);

	float c_subsurface = 0.7f;
	float ior = 1.52;
	...
	Cloth_Diffuse() {}
	Cloth_Diffuse(float roughness_, float w_) : roughness(roughness_), w(w_) {}
	...
	float f_v_h(float& D_velvet, float& roughness2, float& cos_i, float& cos_o, float& cot2h, float& sin4h)
	{
		return D_velvet_neubel(roughness2, cot2h, sin4h) / (4.0f * (cos_i + cos_o - cos_i * cos_o));
	}
...
color *= (lightIntensity * lightAttenuation * NoL) * lightColor;
	...
	*/
	...
	vec3 eval(const vec3& direction_in, const vec3& direction_out, const vec3& n, vec3& original_n, vec3& color)
	{
		...
		/*float F = Dielectric_Reflectance(ior, cos_i);

		return color * ipi * (1.0f - F) * clampf(cos_o + w_constant) * clampf(c_subsurface + cos_o);*/

		vec3 h = (-direction_in + direction_out).norm();

		float cos_o_h = direction_out.dot(h);

		float cos_o_h2 = cos_o_h * cos_o_h;

		float sin_o_h2 = 1.0f - cos_o_h2;

		float sin_o_h4 = sin_o_h2 * sin_o_h2;

		float D = Charlie_D(cos_o_h);
		float V = Charlie_G(cos_i, cos_o);
		vec3 Fr = D * V * Sheen_Color;

		float cot2h = cos_o_h2 / sin_o_h2;


		float fvh = f_v_h(D, roughness_2, cos_i, cos_o, cot2h, sin_o_h4);

		vec3 diffuse = color * ipi * (1.0f - fvh);

		diffuse *= (cos_o + 0.5f) / 2.25f;

		vec3 Fd = diffuse * (c_subsurface + cos_o);
...
class DieletricLambert : public BSDF_Sampling
{
public:
	...
	bool sample(const vec3& direction_in, vec3& n, vec3& original_n, Sampling_Struct& sampling, bool& isReflect, vec3& color)
	{
		...
		Fi = Dielectric_Reflectance(eta, cos_i, cos_t);

		if (randf() < Fi)//Reflect
		{
			vec3 direction_out = direction_in - 2.0f * cos_i * n;
			sampling = Sampling_Struct(direction_out, vec3(1.0f), Fi, true);
			isReflect = true;
			return true;
		}
		else
		{
			//vec3 direction_out = (eta * direction_in + (eta * cos_i - cos_t) * n).norm();

			vec3 sample_direction = cosine_hemisphere_Sampling_with_n(n);

			float cos_o = sample_direction.dot(n);

			float cos_o_t;

			float Fo = Dielectric_Reflectance(ior, cos_o_t);

			if (randf() < Fo)//only allow 1 reflectance for now
			{
				vec3 direction_out = sample_direction + 2.0f * cos_o * n;
				sampling = Sampling_Struct(direction_out, color, Fo, true);
				isReflect = true;
				return true;
				//return false;
			}
			...
			{
				vec3 direction_out = Refract(sample_direction, n, ior, cos_o, cos_t);
				isReflect = true;

				float shift = thickness / cos_o;

				//(x - ox) ^ 2 + (y - oy) ^ 2 + (z - oz) ^ 2 = l
				//boi vi r.d da normalize

				//r.d * t = l
				//t = l

				vec3 weight = color * cos_o * ipi;//(1.0f - Fi) * (1.0f - Fo) * color * cos_o * ipi;
				float pdf = (1.0f - Fi) * (1.0f - Fo) * cos_o * ipi;

				sampling = Sampling_Struct(direction_out, weight, pdf, true);
				sampling.ray_shift = shift;
				return true;
			}
		}
